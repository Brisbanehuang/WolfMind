...
class GameLogger:
    ...
    def log_message(self, category: str, content: str, player_name: Optional[str] = None):
        ...
        with open(self.log_file, 'a', encoding='utf-8') as f:
            if player_name:
                f.write(f"[{timestamp}] [{category}] {player_name}: {content}\n")
            else:
    ...
    def log_vote_result(self, result: str, votes_detail: str, vote_type: str = "投票结果", action: str = "被选中击杀"):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S")
        with open(self.log_file, 'a', encoding='utf-8') as f:
            f.write(f"[{timestamp}] [{vote_type}] {result} {action} (票数: {votes_detail})\n\n")
    ...
    def close(self):
        ...
        with open(self.log_file, 'a', encoding='utf-8') as f:
            f.write("\n" + "=" * 80 + "\n")
            f.write(f"游戏结束时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("=" * 80 + "\n")
